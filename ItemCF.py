...
class ItemCF():
            ...
            for movie1,rate1 in value.items():
                ...
                for movie2,rate2 in value.items():
                    ...
                    if movie1 == movie2:
                        ...
                        if movie2 not in self.sim_dic[movie1].keys():
                            #a = input('cal第二部分循环，不相等第一次')##########################
                            self.sim_dic[movie1].setdefault(movie2,[])
                            if float(rate1) == self.user_aver_rate[user]:
                                rate1 = float(rate1)+0.00001      # in case denominator = 0
                            if float(rate2) == self.user_aver_rate[user]:
                                rate2 = float(rate2)+0.00001      # in case denominator = 0
                            p1 = (float(rate1) - self.user_aver_rate[user])**2
                            p2 = (float(rate2) - self.user_aver_rate[user])**2
                            p0 = (float(rate1) - self.user_aver_rate[user])*(float(rate2) - self.user_aver_rate[user])
    ...
    def estimation(self):
        ...
        print(f' pearson coefficient : prscision is {precision} and recall is {recall}',file = sys.stderr)###################################
